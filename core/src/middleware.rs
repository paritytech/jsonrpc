...
pub trait Middleware<M: Metadata>: Send + Sync + 'static {
	...
	type Future: Future<Output = Option<SerializedResponse>> + Send + 'static;

	/// A returned call future.
	type CallFuture: Future<Output = Option<SerializedOutput>> + Send + 'static;
	...
	where
		...
		X: Future<Output = Option<SerializedOutput>> + Send + 'static,
	{
...
pub type NoopFuture = Pin<Box<dyn Future<Output = Option<SerializedResponse>> + Send>>;
/// Dummy future used as a noop call result of middleware.
pub type NoopCallFuture = Pin<Box<dyn Future<Output = Option<SerializedOutput>> + Send>>;
